...
namespace Gut
{
	...
	// ─────────────────────────────────────────────────────────────────────────
	//  getLastestRows  — returns the most-recent candle for symbol+interval
	// ─────────────────────────────────────────────────────────────────────────
	std::vector<StockData> Price_data_db_helper::getLastestRows(String &symbol,
																const std::string &interval,
																uint32_t limit)
	{
		const char *sql =
			"SELECT date, open, high, low, close, volume FROM price_history "
			"WHERE ticker = ? AND interval = ? ORDER BY date DESC LIMIT ?;";
		...
		std::cout << "[DB] getLastestRows: fetched " << results.size() << " rows for " << symbol
				  << " [" << interval << "]" << std::endl;
		return results;
	}
	...
	StockData::StockData(uint64_t ts, double open, double close, double low,
							  double high, uint64_t volume) : ts(ts), open(open), close(close), low(low), high(high), volume(volume) {}

	String StockData::messageFormat()
	{
		String content;
		content.reserve(48);
		append_bytes(content, htonll(ts));
